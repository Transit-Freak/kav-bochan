    ...
    <div className="card">
      ...
      {mon === "2012" && (
        <p className="pdesc">🕰️ רישום התחנות של 2012: 32,987 תחנות מקובץ ה-GTFS של משרד התחבורה מ-2012, כפי שיובאו ל-<a href="https://www.openstreetmap.org/changeset/14265835" target="_blank" rel="noopener">OpenStreetMap</a> (רישיון ODbL).
          לכל תחנה: השם, הכתובת והמיקום של אז, ואחריהם מה השתנה באותו מק״ט עד תחילת התיעוד שלנו במרץ 2017 — שינוי שם, הזזה (30 מ׳ ומעלה), או ביטול. מק״ט שנמצא מ-2017 במרחק קילומטר ומעלה נחשב מק״ט שהוקצה מחדש לתחנה אחרת, לא הזזה. מק״ט של 7 ספרות שמתחיל ב-99 הוא מספר זמני שמשרד התחבורה נתן בקובץ 2012 לתחנה בלי מספר שלט; 2,516 כאלה שאף קו לא עצר בהן ב-2012 אינן מוצגות (רשומות בקובץ בלי שירות), ו-84 שכן שירתו קווים מוצגות עם הסימון ״מק״ט זמני״.
  ...
  const stopGroups = useMemo(() => {
    if (is2012) {
      ...
      for (const code in snap12) {
        ...
        if (code.length === 7 && code.startsWith("99") && !(snap12[code][10] || 0)) continue;   // מספר זמני בלי אף קו ב-2012
        const a = (addr || "").trim();
