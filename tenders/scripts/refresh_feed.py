...
QUERIES=[word+':updateDate:officeName:משרד התחבורה והבטיחות בדרכים' for word in ('קווי','אוטובוסים','מוניות','רישיונות','הפעלת')]
...
def main():
 ...
 try:
  ...
  for query in QUERIES:
   records,total,pages=fetch_page(0,query)
   if pages>50:raise ValueError('Search exceeds bounded page limit; preserve prior feed')
   for n in range(1,pages):
    batch,t,p=fetch_page(n,query)
   ...
   unique.update({x['id']:x for x in records});reports.append({'query':query,'pages':pages,'records':total})
  records=list(unique.values());total=len(records);pages=sum(r['pages'] for r in reports)
  feed=merge(old,records,now);feed['queries']=reports
  feed['coverage']='חמישה חיפושים במפרסם משרד התחבורה; ייתכנו ניסוחים או מקורות נוספים.'
  feed['lastRun']={'ok':True,'pages':pages,'records':total}
