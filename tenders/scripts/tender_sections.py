...
def simplify(s):
    ...
    s = re.sub(r'(החברה (?:שתזכה|המתמודדת))( במכרז)? י([א-ת]{2,})', r'\1\2 ת\3', s)
    s = re.sub(r'(?<![א-ת])י(?!די |ום |מי |תר |חד |ותר |עד |שראל)([א-ת]{2,}) (החברה (?:שתזכה|המתמודדת))', r'ת\1 \2', s)    # "ישלים החברה שתזכה" → "תשלים החברה שתזכה"; לא "על ידי החברה"
