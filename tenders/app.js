...
function renderDetail(){const t=tenders.find(t=>t.id===selected);if(!t){$('detail').innerHTML='<h2>אין תוצאות להצגה</h2><p>גרסת הניסיון כוללת שני מכרזים בלבד. רשימות הקווים טרם הושלמו.</p>';return;}
 ...
 $('detail').innerHTML=`<span class="tag">${esc(t.status)}</span><h2>${esc(t.title)}</h2><p class="muted">${esc(t.publisher)} · מכרז <bdi>${t.number}</bdi></p><p>${esc(t.description)}</p><div class="facts"><div><span>פרסום המסמך</span><bdi>${t.date}</bdi></div><div><span>מועד הגשה</span><bdi>${t.deadline}</bdi></div><div><span>זוכה</span><b>${t.winner}</b></div><div><span>מצב הבדיקה</span><b>חלק מהפרטים עדיין בבדיקה</b></div></div><nav class="tabs" aria-label="פרטי המכרז">${[['routes','קווים'],['conditions','תנאים עיקריים'],['sources','עדכונים ומקורות']].map(([id,title])=>`<button data-tab="${id}" aria-pressed="${tab===id}">${title}</button>`).join('')}</nav>${content}`;document.querySelectorAll('[data-tab]').forEach(b=>b.onclick=()=>{tab=b.dataset.tab;renderDetail()});}
...
function formatFieldValue(f){
 ...
 if(f.unit)value+=(['years','months','days'].includes(f.unit)?' ':' · ')+(unitLabels[f.unit]||f.unit);
...
document.addEventListener('click',event=>{
 ...
 routeDialog.innerHTML=`<form method="dialog"><button>סגירה ✕</button></form><h2 id="route-title">קו ${esc(route.number)} · ${esc(route.area)}</h2><p>${esc(route.description)}</p><p class="warning">תכנון לפי מכרז 2014; אינו מסלול נוכחי. תחנות, כיוונים וחלופות עדיין לא נבדקו.</p>${reviewSource(review,route.page)}`;
 routeDialog.showModal();
});
