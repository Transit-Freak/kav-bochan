...
const CATEGORIES = ['אזורי', 'בינעירוני ארוך', 'בינעירוני קצר', 'עירוני תדירות גבוהה', 'עירוני תדירות נמוכה', 'לילה', 'קווים מזינים', 'תלמידים'];
...
function ChoiceScreen({
  ...
  }, "\u05E9\u05E0\u05D9\u05DD-\u05E2\u05E9\u05E8 \u05DB\u05DC\u05D9\u05DD \u05DC\u05E0\u05D9\u05EA\u05D5\u05D7 \u05D4\u05EA\u05D7\u05D1\u05D5\u05E8\u05D4 \u05D4\u05E6\u05D9\u05D1\u05D5\u05E8\u05D9\u05EA \u2014 \u05D1\u05DE\u05D4 \u05DC\u05D1\u05D7\u05D5\u05E8?"), /*#__PURE__*/React.createElement("button", {
...
function GoldenApp({
  ...
  }, "\u05E0\u05EA\u05D5\u05E0\u05D9 \u05E0\u05D5\u05E1\u05E2\u05D9\u05DD \u05D5\u05E2\u05DC\u05D5\u05D9\u05D5\u05EA: \u05E6\u05D9\u05DC\u05D5\u05DD \u05DE\u05E9\u05E8\u05D3 \u05D4\u05EA\u05D7\u05D1\u05D5\u05E8\u05D4, \u05D9\u05D5\u05E0\u05D9 2026", liveGen ? ` · הצלבה מול רישום הקווים העדכני: ${String(liveGen).split('-').reverse().join('.')}` : '')), /*#__PURE__*/React.createElement("button", {
    type: "button",
    onClick: onBack,
    className: "kb-home",
  ...
  focusMakat && trips.some(t => String(t.makat || '').replace(/^0+/, '').trim() === String(focusMakat).replace(/^0+/, '').trim()) ? /*#__PURE__*/React.createElement("div", {
    ...
    if (!selectedLine) {
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        className: "text-xs font-bold text-rose-500 text-right mt-4"
...
function KavPach() {
  ...
  useEffect(() => {
    fetch('bus/data/deadhead.json', {
      cache: 'no-cache'
    }).then(r => r.ok ? r.json() : null).then(d => d && setDhObs(d)).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    fetch('kavpach-overlap.json', {
      cache: 'no-cache'
    }).then(r => r.ok ? r.json() : null).then(d => d && setOverlapMap(d.lines || null)).catch(() => {});
  }, []);
  ...
  const loadFromXLSX = useCallback(async () => {
    ...
    try {
      ...
      if (jsonMainRes && jsonMainRes.ok) {
        ...
        const [jsonScheduleBuf, jsonStopsBuf, jsonBenchmarkBuf] = await Promise.all([grabWithProgress('data-schedule.json', false, b => {
          jsonReceived.schedule = b;
          updateJsonProgress();
        }), grabWithProgress('data-stops.json', false, b => {
          jsonReceived.stops = b;
          updateJsonProgress();
  ...
  const redundantLines = useMemo(() => {
    ...
    return Object.entries(groups).map(([groupKey, data]) => {
      ...
      if ((category === 'עירוני תדירות גבוהה' || category === 'עירוני תדירות נמוכה') && totalTrips >= 500 && totalTrips <= 700) {
        ...
        lowRiderTh = Math.round(LOW_RIDER_THRESHOLD['עירוני תדירות נמוכה'] + (LOW_RIDER_THRESHOLD['עירוני תדירות גבוהה'] - LOW_RIDER_THRESHOLD['עירוני תדירות נמוכה']) * w);
      }
      ...
      let fCost = 0;
      if (costRatio === 0) fCost = 0;else if (costRatio <= 0.7) fCost = 0;else if (costRatio <= 1.3) fCost = 0.25;else if (costRatio <= 1.7) fCost = 0.4;else if (costRatio <= 2.5) fCost = 0.6;else if (costRatio <= 4) fCost = 0.8;else if (costRatio <= 6) fCost = 0.9;else fCost = 1;
      ...
      let fRiders = 0;
      if (avgRiders < lowFull) fRiders += 0.5;else if (avgRiders < lowHalf) fRiders += 7 / 30;
      ...
      Object.keys(componentScores).forEach(k => {
        componentScores[k] = Math.round(componentScores[k]);
      });
      ...
      if (PP.exclusive > 0 && (exclusiveStops > 0 || isExclusiveDest)) {
        protections.push({
          name: 'תחנות ייחודיות',
          value: PP.exclusive,
          detail: exclusiveStops > 0 ? `${exclusiveStops} תחנות בלעדיות` : 'יעד יחיד באזור'
        });
      ...
      if (PP.train > 0 && isTrainCoord) {
        protections.push({
          name: 'מותאם רכבת',
          value: PP.train,
          detail: 'יוצא בתיאום עם לוז רכבת'
        });
      ...
      if (PP.school > 0 && category === 'תלמידים') {
        ...
        const schoolHourTrips = data.filter(t => t.timeMins >= 420 && t.timeMins <= 510 || t.timeMins >= 780 && t.timeMins <= 930).reduce((s, t) => s + t.tripCount, 0);
        const schoolRatio = totalTrips > 0 ? schoolHourTrips / totalTrips : 0;
        if (schoolRatio >= 0.6) {
          protections.push({
            name: 'תלמידים בשעות בי"ס',
            value: PP.school,
            detail: `${Math.round(schoolRatio * 100)}% מהנסיעות`
          });
      ...
      if (PP.prebook > 0 && data[0].isEilatPrebooked) {
        protections.push({
          name: 'הזמנה מראש',
          value: PP.prebook,
          detail: 'התיקופים חלקיים — העומס בפועל גבוה מהנמדד'
        });
      ...
      if (PP.weekend > 0 && totalTrips > 0 && wkndTrips / totalTrips >= 0.6) {
        protections.push({
          name: 'קו סופ"ש',
          value: PP.weekend,
          detail: `${Math.round(100 * wkndTrips / totalTrips)}% מהנסיעות בשישי-שבת`
        });
      ...
      const firstAlive = data.find(t => {
        const lv = liveOf(t.makat);
        return !(lv && lv.rm);
      });
      ...
      if (live && !live.rm) {
        ...
        if (PP.newLine > 0 && live.newd && !live.gap) {
          protections.push({
            name: 'קו חדש בהרצה',
            value: PP.newLine,
            detail: `הופיע לראשונה ב-${String(live.newd).split('-').reverse().join('.')}`
          });
        ...
        if (PP.reduced > 0 && (live.red || 0) >= 2) {
          protections.push({
            name: 'כבר צומצם',
            value: PP.reduced,
            detail: `${live.red} צמצומי שירות בשנה האחרונה`
          });
      ...
      const annualExcess = avgCost > 0 && costBenchmark > 0 && avgCost > costBenchmark ? Math.round((avgCost - costBenchmark) * avgRiders * totalTrips * 52) : 0;
  ...
  const fullLoadingScreen = /*#__PURE__*/React.createElement("div", {
    className: "min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-center gap-6 px-6",
    dir: "rtl",
    style: {
      fontFamily: "'Heebo', sans-serif"
    }
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "kb-home",
    title: "\u05D7\u05D6\u05E8\u05D4 \u05DC\u05E2\u05DE\u05D5\u05D3 \u05D4\u05E8\u05D0\u05E9\u05D9"
  }, "\u2190 \u05D7\u05D6\u05E8\u05D4 \u05DC\u05E7\u05D5 \u05D4\u05D1\u05D5\u05D7\u05DF")), fileLoad.active || initialLoading ? /*#__PURE__*/React.createElement("div", {
    ...
    className: "text-slate-500 font-bold text-sm"
  }, fileLoad.progress, "%"))) : trips.length === 0 && csvLoadFailed ? /*#__PURE__*/React.createElement("div", {
  ...
  })(), tab === "allTrips" && /*#__PURE__*/React.createElement("div", {
  ...
  }, "\u05D4\u05D5\u05E6\u05D2\u05D5 \u05DB\u05DC ", tableTrips.length.toLocaleString(), " \u05D4\u05EA\u05D5\u05E6\u05D0\u05D5\u05EA."))), tab === "simulator" && /*#__PURE__*/React.createElement("div", {
    className: "bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm max-w-4xl mx-auto transition-opacity duration-300 opacity-100"
  }, /*#__PURE__*/React.createElement("header", {
    className: "mb-8"
  }, /*#__PURE__*/React.createElement("h2", {
