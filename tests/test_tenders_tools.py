...
def test_brief_is_the_key_sentence_in_plain_words():
    ...
    assert tender_sections.brief_of('52.2.1 ' + t, '', ['תמורה ותשלומים']).startswith('52.2.1 המפעיל יקבל תשלום על הצטיידות באוטובוסים כך: 52.2.1.1 על אוטובוס עירוני סך של 155,000 ₪.')
    ...
    assert tender_sections.simplify('במשך כל תקופת ההתקשרות בהתאם להליך תחרותי זה, בהתאם לאמור בסעיף 5.') == 'במשך כל תקופת ההתקשרות לפי מכרז זה.'
...
def test_numbers_come_with_their_context_and_cross_references_are_dropped():
    ...
    assert nums['100 אלף ₪'] == 'פיצוי מוסכם עד 100 אלף ₪ על כל שבוע איחור'
...
def test_old_pdf_quotes_are_tidied_without_touching_good_ones():
    ...
    assert f('בנוסף, קו 468 ממודיעין )10014( 14 ימשיך; מעלה אדומים ( ,)209בית אל (,)269') == \
        'בנוסף, קו 468 ממודיעין (10014) 14 ימשיך; מעלה אדומים (209), בית אל (269),'
