...
SPECIAL_RE = re.compile(r'נעים\s*בסופ|סופ["\'״׳]?ש\b|בסופש|סובב\s|שאטל|שבתון|קו\s*שבת|קווי\s*שבת')
...
def street_norm(t):
    ...
    t = re.sub(r"(?<![\s|!])''(?!')", '', t)                    # גרשיים בתוך מילה (התע''ש) — לא סימון מודגש
    t = re.sub(r'\s*\([^)]*\)', ' ', t)                        # (דימונה)
    t = CLEAN.sub(' ', t).strip()
    t = STREET_PFX.sub('', t)
    # גרשיים וגרש נמחקים בלי רווח (התע''ש → התעש, בי''ח → ביח) — לא כמו CLEAN שמחליף ברווח
